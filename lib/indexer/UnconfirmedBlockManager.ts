...
export class UnconfirmedBlockManager {
  ...
  constructor(dbService: DatabaseService, blockTable: any, confirmedHeight: number) {
  ...
  async initialize() {
    ...
    .where("block.blockNumber > :confirmedHeight", { confirmedHeight: this.confirmedHeight });
