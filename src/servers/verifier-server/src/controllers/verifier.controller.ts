...
export class VerifierController {
  ...
  @Post("prepare")
  public async check(@Body() request: ARType): Promise<ApiResponse<any>> {
    return handleApiResponse(this.processor.prepareRequest(request));
  }

  @Post("integrity")
  public async getIntegrity(@Body() request: ARType): Promise<ApiResponse<string>> {
    return handleApiResponse(this.processor.getMessageIntegrityCheck(request));
  }

  @Post("prepareAttestation")
  public async getAttestationData(@Body() request: ARType): Promise<ApiResponse<string>> {
