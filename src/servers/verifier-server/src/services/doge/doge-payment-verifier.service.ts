...
export class DOGEPaymentVerifierService {
    ...
    private async verifyRequest(request: Payment_RequestNoMic | Payment_Request): Promise<AttestationResponse<Payment_Response>> {
        if (request.attestationType !== encodeAttestationName("Payment") || request.sourceId !== encodeAttestationName("DOGE")) {
            throw new HttpException(
                {
                    status: HttpStatus.BAD_REQUEST,
                    error: `Attestation type and source id combination not supported: (${request.attestationType}, ${request.sourceId}). This source supports attestation type 'Payment' (0x5061796d656e7400000000000000000000000000000000000000000000000000) and source id 'DOGE' (0x444f474500000000000000000000000000000000000000000000000000000000).`,
                },
        ...
        const result = await verifyPayment(DogeTransaction, fixedRequest, this.processor.indexedQueryManager, this.processor.client);
