...
AxiosRateLimit.prototype.handleResponse = function (response) {
  this.inProcessing -= 1;
  if (this.onResponse) {
    setTimeout(() => { this.onResponse(this.inProcessing, this.queue.length); }, 0)
  }
  this.shift()
  return response
}
...
AxiosRateLimit.prototype.shift = function () {
  if (!this.queue.length) {
    if (this.onQueueEmpty) {
      setTimeout(() => { this.onQueueEmpty(this.inProcessing, this.queue.length, this.maxRequests); }, 0);
    }
    return
  }
  if (this.timeslotRequests === this.maxRequests) {
    ...
    if (this.onLimitReached) {
      setTimeout(() => { this.onLimitReached(this.inProcessing, this.queue.length, this.maxRequests); }, 0);
    }
    return
  }
  ...
  if (this.onSend) {
    setTimeout(() => { this.onSend(this.inProcessing, this.queue.length, this.maxRequests); }, 0);
  }
  queued.resolve()
...
AxiosRateLimit.prototype.onRetry = function (retryCount) {
  if (this.onRetry) {
    setTimeout(() => { this.onRetry(retryCount); }, 0);
  }
  return 0;
}
