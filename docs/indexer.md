...
When querying, the mandatory input into the query is the hash of the lowest confirmation block. We indicate the hash as  `dataAvailabilityProof`. Given the `dataAvailabilityProof`, a block with this hash is first determined from block database. Let `H` be the height of this block, then the upper query boundary is defined by `H - numberOfConfirmations + 1`. Surely, blocks of such height are confirmed.
...
- a block with hash `dataAvailabilityProof` does not exist in the database,
- a block with hash `dataAvailabilityProof` exists in the database, but database does not contain all the confirmed blocks, in particular the ones at the height `H - numberOfConfirmations + 1`.
...
- a block with hash `dataAvailabilityProof` has not yet arrived to our blockchain node, so the indexer has not read it yet. We should wait a bit more.
- there is no block with hash `dataAvailabilityProof`. We should reject the attestation that triggered the query.

The main idea for reaching a synchronized upper boundary is the following. If an attestation request is valid (can be confirmed), then the confirmation block with hash `dataAvailabilityProof` must exist, such a block has been mined and the proposer must have seen it. Except in the case of "selfish mining" as described below, such a block is either being distribed or has been already distributed throughout the blockchain peer nodes. Hence an attestation provider should just wait for say 30s from the end of the `collect` phase in which the associated request was sent, and it should receive it with very high probability. So the correct behaviour that always works with high probability is: if you already have the block with hash `dataAvailabilityProof`, you are safe to make a query and proceed with other checks for confirming/rejecting the attestation request. On the other hand, if you do not have such a block, wait 30s and then retry. In case `dataAvailabilityProof` is wrong, nobody will find a block with such a hash in the database, hence everybody will reject the attestation request due to a wrong `dataAvailabilityProof`.
...
- Read the block with the hash `dataAvailabilityProof` from the database.
- If such a block does not exist, delay the query to the second step (finish this step).
- Otherwise, let `H` be the block number (height) for the block with the hash `dataAvailabilityProof`. Let `U = H - numberOfConfirmations + 1`.
...
- Read the block with the hash `dataAvailabilityProof` from the database.
- If such a block does not exist, we first verify, if our aggressive reading of blocks works well. Indexer is trying to read new top blocks aggressively, and everytime it makes a check, it stores a timestamp into the database. 
  ...
  - Otherwise, everything is ok with the indexer and we can be sure with very high probability, that a block with the hash `dataAvailabilityProof` does not exist, the attestation request is invalid.
- Otherwise, such a block exists. let `H` be the block number (height) for the block with the hash `dataAvailabilityProof`. Let `U = H - numberOfConfirmations + 1`.
- Read the last confirmed block number `N` in the indexer.
- If `N >= U`, then proceed with the query and the upper boundary `U`.
