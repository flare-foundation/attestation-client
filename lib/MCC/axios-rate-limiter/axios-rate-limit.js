  ...
  AxiosRateLimit.prototype.shift = function () {
    if (!this.queue.length) {
        if(this.onQueueEmpty) {
            setTimeout(()=>{this.onQueueEmpty(this.inProcessing, this.queue.length);}, 0);
        }
        return
    }
    if (this.timeslotRequests === this.maxRequests) {
      ...
      if(this.onLimitReached) {
          setTimeout(()=>{this.onLimitReached(this.inProcessing, this.queue.length);}, 0);
      }
      return
    }
    ...
    if(this.onSend) {
        setTimeout( ()=>{this.onSend(this.inProcessing, this.queue.length);}, 0);
    }
    queued.resolve()
