    ...
    async perf(): Promise<PerformanceInfo[]> {
        ...
        if (this.disks) {
            for (let disk of this.disks) {
                ...
                resArray.push(new PerformanceInfo(`system.disk.${disk.filesystem}`, `available`, round(disk.available / (1024 * 1024), 3), "GB", `${round(disk.available * 100 / total, 1)}% available`));

                resArray.push(new PerformanceInfo(`system.disk.${disk.filesystem}`, `used`, round(disk.used / (1024 * 1024), 3), "GB", `${round(disk.used * 100 / total, 1)}% used`));
            }
        }
