...
interface IAttestationClient {
   ...
   struct BalanceDecreasingTransactionProof {
      // Round number (epoch id) of the state connector request
      uint256 stateConnectorRound;
   ...
   struct ReferencedPaymentNonexistenceProof {
      // Round number (epoch id) of the state connector request
      uint256 stateConnectorRound;
   ...
   // (possible values are defined in attestation specs).
    ...
    function verifyBalanceDecreasingTransactionProof(uint32 _chainId, BalanceDecreasingTransactionProof calldata _data) 
       external view
       returns (bool _proved);
    function verifyBlockHeightExistsProof(uint32 _chainId, BlockHeightExistsProof calldata _data) 
       external view
       returns (bool _proved);
    function verifyReferencedPaymentNonexistenceProof(uint32 _chainId, ReferencedPaymentNonexistenceProof calldata _data) 
       external view
       returns (bool _proved);  
}
